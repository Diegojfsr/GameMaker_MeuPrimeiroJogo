...
function src_player_blue_andando(){
...
// Movimentacao do player
	...
	if place_meeting(x+hveloc, y, obj_mid_left){
		while !place_meeting(x+sign(hveloc), y, obj_mid_left){
		x+= sign(hveloc);
		}
	hveloc = 0;
	}
	...
	//Colisao player parede
	if place_meeting(x, y  +vveloc, obj_mid_left){
		while !place_meeting(x, y + sign(hveloc), obj_mid_left){
		y += sign(vveloc);
		}
	vveloc = 0;
	}
...
function src_player_blue_ativa_dash(){
	// Codigo que puxa a acao da function Dash
	if mouse_check_button_pressed(mb_right){
		alarm[0] = 25;
		dash_dir = point_direction(x, y, mouse_x, mouse_y);
		estado = src_player_blue_dash;
	}
}

// Funcao para criar o Dash
function src_player_blue_dash(){
...
function src_player_blue_qtd_dash(){
...
function src_player_blue_atirando(){
	// Shooting
...
function src_player_blue_municao(){
	// Controla a municao
	if alarm[1] <= 0{
		municao += 1;
	}
